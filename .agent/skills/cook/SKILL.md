...
- **Finalize (MANDATORY - never skip):**
  1. `project-manager` skill → update plan/phase status to complete
  2. `docs-manager` skill → update `./docs` if changes warrant
  3. `TaskUpdate` → mark all Claude Tasks complete
  4. Ask user if they want to commit via `git-manager` skill

## Required Skills (MANDATORY)

| Phase | Skill | Requirement |
|-------|----------|-------------|
...
- Steps 4, 5, 6 **MUST** use Task tool to spawn parallel tasks
- DO NOT implement testing, review, or finalization yourself - DELEGATE
- If workflow ends with 0 Task tool calls, it is INCOMPLETE
- Pattern: `Task(skill_type="[type]", prompt="[task]", description="[brief]")`
...
- `references/skill-patterns.md` - Skill invocation patterns
