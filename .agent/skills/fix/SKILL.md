...
**First action:** If there is no "auto" keyword in the request, use `notify_user` to determine workflow mode:
...
See `references/mode-selection.md` for notify_user format.

### Step 2: Debug

- Activate `debug` skill.
- Guess all possible root causes.
- Spawn multiple `Explore` parallel tasks in parallel to verify each hypothesis.
...
- Spawn multiple `Explore` parallel tasks to find possible related code for verification.
...
2. `docs-manager` skill → update `./docs` if changes warrant (NON-OPTIONAL)
3. `TaskUpdate` → mark all Claude Tasks complete
4. Ask user if they want to commit via `git-manager` skill

---

## IMPORTANT: Skill/Skill Activation Matrix
...
**Skills:** `debugger`, `researcher`, `planner`, `code-reviewer`, `tester`, `Bash`
...
Load as needed:
- `references/mode-selection.md` - notify_user format for mode
...
- `references/parallel-exploration.md` - Parallel Explore/Bash parallel tasks patterns
