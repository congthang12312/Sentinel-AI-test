...
- Context-efficient: parallel tasks handle MCP discovery and execution
...
5. **Context Management**: Avoiding context pollution by delegating MCP operations to parallel tasks
...
- Faster than skill orchestration
...
### Pattern 2: Skill-Based Execution (Fallback)

Use `mcp-manager` agent when Gemini CLI unavailable. Skill discovers tools, selects relevant ones, executes tasks, reports back.
...
3. **mcp-manager Skill** (Fallback): Context-efficient delegation
