...
**IMPORTANT:**
- Skills are not documentation, they are practical instructions for the AI agent to use the tools, packages, plugins or APIs to achieve the tasks.
- Each skill teaches Claude how to perform a specific development task, not what a tool does.
- the AI agent can activate multiple skills automatically to achieve the user's request.
...
- Descriptions in metadata of `SKILL.md` files should be both concise (**less than 200 characters**) and still contains enough usecases of the references and scripts, this will help skills can be activated automatically during the implementation process of the AI agent.
...
**Why?**
Better **context engineering**: leverage **progressive disclosure** technique of Agent Skills, when agent skills are activated, the AI agent will consider to load only relevant files into the context, instead of reading all long `SKILL.md` as before.
...
**IMPORTANT:**
- Referenced markdown files should be also **less than 150 lines**, remember that you can always split them into multiple files (**progressive disclosure** principle).
- Referenced markdown files are practical instructions for the AI agent to use the tools, packages, plugins or APIs to achieve the tasks.
- Each skill teaches Claude how to perform a specific development task, not what a tool does.
...
Use `notify_user` tool to gather user feedback and validate understanding.
...
If you receive a lot of URLs or files, use multiple `WebFetch` tools and `Explore` parallel tasks to explore them in parallel, then report back to main agent.
