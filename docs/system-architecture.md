...
**Project**: Antigravity Kit

## Overview

Antigravity Kit implements a multi-agent AI orchestration architecture where specialized agents collaborate through a file-based communication protocol. The system enables developers to leverage AI assistance throughout the entire software development lifecycle - from planning and implementation to testing, review, and deployment.
...
#### 1.1 CLI Interface
**Location**: Antigravity / Antigravity CLI
...
**Technology**: Antigravity / Antigravity
...
**Planning Agents**:
- `planner` - Technical planning and architecture (Claude Opus 4.6 Thinking)
...
- `docs-manager` - Documentation maintenance (Gemini 3 Pro High)
...
**Agent Modes**:
- `skill`: Spawned by other agents, runs independently
- `all`: Can be invoked as main or sub agent

**Model Selection**:
- `default-model` - Fast, efficient (primary)
- `advanced-model` - Deep reasoning (complex planning)
- `cost-effective-model` - Lightweight tasks
- `specialized-model` - Git operations
...
**Command Directories** (`.agent/workflows/`):
...
**Purpose**: Intercept and control Antigravity operations for performance, context management, and security

**Hook Architecture**:
All hooks located in `.agent/hooks/` with consistent patterns - fail-safe exit code 0 (non-blocking)
...
- **Functionality**:
  - Injects current development rules from `.agent/rules/`
...
**3. Skill-Init Hook** (`skill-init.cjs`)
- **Trigger**: When spawning parallel tasks
- **Purpose**: Provide minimal context to parallel tasks
- **Functionality**:
...
**Hook Configuration** (`.agent/settings.json`):
```json
{
  "hooks": {
    "SkillStart": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "node ${CLAUDE_PROJECT_DIR}/.agent/hooks/skill-init.cjs"
      }]
    }],
    "BeforeBash": [{
      "type": "command",
      "command": "node ${CLAUDE_PROJECT_DIR}/.agent/hooks/scout-block.js"
    }]
  }
}
```
...
**Architecture**:
```
.agent/skills/markdown-novel-viewer/
├── scripts/
...
**AI Platforms & Model Selection**:

| Role | Model nên dùng | Lý do |
|------|----------------|-------|
| Most agents (fast) | Claude Sonnet 4.5 | Nhanh, code rất mạnh, ổn định (tốt hơn Sonnet 4 cũ) |
| Planner (advanced) | Claude Opus 4.6 (Thinking) | Model mạnh nhất, vượt Opus 4.1 |
| Docs-manager | Gemini 3 Pro (High) | Đọc docs cực tốt, context lớn (Google DeepMind) |
| Git-manager | Claude Sonnet 4.5 (Thinking) | Reasoning tốt hơn bản thường, rất mạnh cho refactor |
...
1. Create agent definition file: `.agent/skills/my-agent.md`
...
1. Create command file: `.agent/workflows/my-command.md`
...
1. Create skill directory: `.agent/skills/my-skill/`
...
1. Define workflow in `.agent/rules/`
...
### External Resources
- [Antigravity Documentation](https://docs.claude.com/)
- [Antigravity Documentation](https://opencode.ai/docs)
